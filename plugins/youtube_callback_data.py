    ...
    if cb_data.startswith("ytdata||"):
        ...
        if media_type == 'audio':
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "صوت 🎵", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("مستند 🗂️",
                                                                                                    callback_data=f"docaudio||{format_id}||{yturl}")]])
        else:
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "فيديو 📽️", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("مستند 🗂️",
...
async def catch_youtube_dldata(c, q):
    ...
    await q.edit_message_reply_markup(
        InlineKeyboardMarkup([[InlineKeyboardButton("جارى التحميل...⏳", callback_data="down")]]))
    filepath = os.path.join(userdir, filext)
    # await q.edit_message_reply_markup([[InlineKeyboardButton("معالجة..🔨")]])
...
async def send_file(c, q, med, filename):
    ...
    try:
        await q.edit_message_reply_markup(
            InlineKeyboardMarkup([[InlineKeyboardButton("جارٍ التحميل ...⬇️", callback_data="down")]]))
