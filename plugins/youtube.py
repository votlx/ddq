...
async def ytdl(_, message):
    ...
    try:
        if userLastDownloadTime > datetime.now():
            ...
            await message.reply_text(f"`أنتظر {wait_time} دقائق قبل الطلب التالي`")
            return
    except:
        pass
    ...
    try:
        ...
        await message.reply_text("`فشل إحضار بيانات اليوتيوب ... 😔 \nمحتمل يوتيوب خادم IP المحظور \n#خطا`")
        return
    buttons = InlineKeyboardMarkup(list(create_buttons(formats)))
    sentm = await message.reply_text("معالجة عنوان URL على Youtube 🔎 🔎 🔎⌛")
    try:
